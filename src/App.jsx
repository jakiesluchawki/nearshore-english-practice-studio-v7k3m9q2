...
function LessonPage({ id, progress, updateProgress, navigate }) {
  ...
  useEffect(() => { setSelectedChoice(""); setDialogueDraft(""); setDialogueAnswer(""); setAddedPhrase(lesson.phrases[0] || ""); setCustomSaved(false); setSelectedRating(""); }, [lesson.id]);
  ...
  return <main id="main" className="lesson-page"><section className={`lesson-hero tone-${lesson.moduleColor}`}><div className="lesson-hero-inner page-width"><button className="back-button" onClick={() => navigate("lessons")}><ArrowLeft size={19} /> Ścieżka nauki</button><div className="lesson-number">{String(lesson.id).padStart(2, "0")}</div><span className="lesson-kicker">Moduł {lesson.moduleId} · {lesson.checkpoint ? "checkpoint · " : ""}{lesson.duration} min</span><h1>{lesson.title}</h1><p>{lesson.goal}.</p><div className="lesson-progress-line"><i style={{ width: `${lesson.id}%` }} /><span>{lesson.id} / 100</span></div></div></section><div className="lesson-content page-width">
    ...
    <section className="lesson-block lesson-block--dialogue"><span className="eyebrow">05 · Twoja kolej</span><h2>{practice.dialogueTitle}</h2><p className="dialogue-instruction">{practice.isMessage ? "Przeczytaj kontekst, napisz własną odpowiedź i wstaw ją do wymiany. Dopiero potem zobaczysz przykład z lekcji." : "Przeczytaj kontekst, powiedz odpowiedź na głos, a potem wpisz ją w brakujące miejsce. Dopiero wtedy zobaczysz przykład z lekcji."}</p><div className="dialogue">{dialogueLead.length ? dialogueLead.map(([speaker, text], index) => <div className={`dialogue-line ${speaker === "You" ? "recruiter" : "candidate"}`} key={`lead-${speaker}-${index}`}><span>{speaker}</span><p>{text}</p></div>) : <div className="dialogue-context"><span>Sytuacja</span><p>Twoim celem jest {lesson.goal}.</p></div>}{dialogueAnswer ? <><div className="dialogue-line recruiter dialogue-line--inserted"><span>You · Twoja wersja</span><p>{dialogueAnswer}</p></div>{dialogueFollowUp.map(([speaker, text], index) => { const isRecruiter = speaker === "You"; return <div className={`dialogue-line ${isRecruiter ? "recruiter" : "candidate"}`} key={`follow-up-${speaker}-${index}`}><span>{speaker}</span><p>{text}</p>{isRecruiter && <button onClick={() => speak(text)} aria-label={`Posłuchaj: ${text}`}><Play size={16} weight="fill" /></button>}</div>; })}<div className="dialogue-comparison"><div><span>Przykład z lekcji</span><p>{dialogueModelAnswer}</p></div><button className="inline-audio" onClick={() => speak(dialogueModelAnswer)}><Headphones size={17} /> Posłuchaj przykładu</button></div><button className="dialogue-edit" onClick={editDialogueAnswer}><NotePencil size={17} /> Zmień swoją odpowiedź</button><p className="dialogue-transfer" role="status"><Check size={17} weight="bold" /> Twoja wersja czeka niżej w sekcji „Twoja wersja”.</p></> : <form className="dialogue-compose" onSubmit={insertDialogueAnswer}><label htmlFor={`dialogue-answer-${lesson.id}`}><span>You</span><strong>{practice.isMessage ? "Twoja wiadomość" : "Twoja odpowiedź"}</strong></label><textarea id={`dialogue-answer-${lesson.id}`} value={dialogueDraft} onChange={(event) => setDialogueDraft(event.target.value)} placeholder={practice.isMessage ? "Napisz krótką odpowiedź po angielsku…" : "Wpisz to, co powiedziałaś po angielsku…"} /><div className="dialogue-compose-actions"><small>Tu nie ma oceny. Porównasz intencję i naturalność.</small><button type="submit" disabled={!dialogueDraft.trim()}>Wstaw do rozmowy <ArrowRight size={17} /></button></div></form>}</div>{dialogueAnswer && <p className="coach-note"><Sparkle size={18} weight="fill" /><span><strong>Coach’s note:</strong> Porównaj intencję i naturalność, nie każde słowo. Jeśli twoja wersja była jasna, krótka i uprzejma, zadziałała.</span></p>}</section>
