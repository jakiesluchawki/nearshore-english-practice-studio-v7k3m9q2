...
function LessonPage({ id, progress, updateProgress, navigate }) {
  const lesson = getLesson(id); const [promptOpen, setPromptOpen] = useState(false); const [selectedChoice, setSelectedChoice] = useState(""); const [addedPhrase, setAddedPhrase] = useState(lesson.phrases[0] || ""); const [customSaved, setCustomSaved] = useState(false); const [selectedRating, setSelectedRating] = useState(""); const phrases = lesson.phrases;
  ...
  useEffect(() => { setSelectedChoice(""); setAddedPhrase(lesson.phrases[0] || ""); setCustomSaved(false); setSelectedRating(""); }, [lesson.id]);
  ...
  return <main id="main" className="lesson-page"><section className={`lesson-hero tone-${lesson.moduleColor}`}><div className="lesson-hero-inner page-width"><button className="back-button" onClick={() => navigate("lessons")}><ArrowLeft size={19} /> Ścieżka nauki</button><div className="lesson-number">{String(lesson.id).padStart(2, "0")}</div><span className="lesson-kicker">Moduł {lesson.moduleId} · {lesson.checkpoint ? "checkpoint · " : ""}{lesson.duration} min</span><h1>{lesson.title}</h1><p>{lesson.goal}.</p><div className="lesson-progress-line"><i style={{ width: `${lesson.id}%` }} /><span>{lesson.id} / 100</span></div></div></section><div className="lesson-content page-width">
    ...
    <section className="lesson-block lesson-block--dialogue"><span className="eyebrow">05 · Mini dialogue</span><h2>{practice.dialogueTitle}</h2><div className="dialogue">{miniDialogue.map(([speaker, text], index) => { const isRecruiter = speaker === "You"; return <div className={`dialogue-line ${isRecruiter ? "recruiter" : "candidate"}`} key={`${speaker}-${index}`}><span>{speaker}</span><p>{text}</p>{isRecruiter && <button onClick={() => speak(text)} aria-label={`Posłuchaj: ${text}`}><Play size={16} weight="fill" /></button>}</div>; })}</div><p className="coach-note"><Sparkle size={18} weight="fill" /><span><strong>Coach’s note:</strong> Nie chodzi o dokładne odtworzenie. Jeśli twoja wersja była jasna, krótka i uprzejma, zadziałała.</span></p></section>
