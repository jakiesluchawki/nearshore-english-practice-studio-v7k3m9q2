...
function Rescue({ updateProgress, navigate }) {
  const rescueLessons = lessons.slice(0, 10).filter((lesson) => !lesson.checkpoint && lesson.phrases.length); const [openId, setOpenId] = useState(2);
...
export function App() {
  ...
  else if (route === "rescue") page = <Rescue updateProgress={updateProgress} navigate={navigate} />;
