...
import {
  ...
  Microphone, NotePencil, Pause, Play, Plus, ShieldCheck, Sparkle, Star, X,
...
import { getDueReviewLessonId, reviewTimingLabel, scheduleReview } from "./data/reviews.js";
...
const starterProgress = { completed: [], phraseRatings: {}, myPhrases: [], reviewSchedule: {}, lastLesson: 1, startedOn: new Date().toISOString(), lastVisit: "", visitDays: [] };
...
function Home({ progress, navigate }) {
  ...
  const nextLessonId = dueReviewLessonId || Math.min(100, completedCount ? Math.max(...progress.completed) + 1 : progress.lastLesson || 1);
...
function PhraseCard({ phrase, index, isSaved, onToggle, onSpeak }) {
  ...
  return <article className="phrase-card"><span className="phrase-label">{label}</span><p>{phrase}</p><div className="phrase-actions"><button onClick={() => onSpeak(phrase)} aria-label={`Odtwórz: ${phrase}`}><Headphones size={18} /> Posłuchaj</button><button className={isSaved ? "saved" : ""} aria-pressed={isSaved} onClick={() => onToggle(phrase)} aria-label={isSaved ? "Usuń z Moich fraz" : "Dodaj do Moich fraz"}><Star size={18} weight={isSaved ? "fill" : "regular"} /> {isSaved ? "Zapisane" : "Zapisz"}</button></div></article>;
}
...
function PromptModal({ lesson, progress, onClose, initialMode = "roleplay", initialInput = "", reviewDialogue = null }) {
  ...
  const prompt = isAnswerReview ? answerReviewPrompt(lesson, input, reviewDialogue) : lessonPrompt(lesson, mode, input, hard, progress.myPhrases.slice(0, 12));
...
function LessonPage({ id, progress, updateProgress, navigate }) {
  ...
  const { answer: quizAnswer, intention: quizIntention, options: choiceOptions } = useMemo(() => getLessonQuiz(lesson), [lesson.id]);
  useEffect(() => { setPromptRequest(null); setSelectedChoice(""); setDialogueDraft(""); setDialogueAnswer(""); setAddedPhrase(lesson.phrases[0] || ""); setCustomSaved(false); setSelectedRating(""); }, [lesson.id]);
  ...
  const rateLesson = (rating) => { const ratings = { ...progress.phraseRatings }; phrases.forEach((phrase) => { ratings[phrase] = rating; }); updateProgress((current) => ({ ...current, completed: [...new Set([...current.completed, lesson.id])].sort((a, b) => a - b), phraseRatings: ratings, reviewSchedule: { ...(current.reviewSchedule || {}), [lesson.id]: scheduleReview(rating) }, lastLesson: Math.min(100, lesson.id + 1) })); setSelectedRating(rating); };
  const insertDialogueAnswer = (event) => { event.preventDefault(); const value = dialogueDraft.trim(); if (!value) return; setDialogueAnswer(value); setAddedPhrase(value); setCustomSaved(false); };
  ...
  return <main id="main" className="lesson-page"><section className={`lesson-hero tone-${lesson.moduleColor}`}><div className="lesson-hero-inner page-width"><button className="back-button" onClick={() => navigate("lessons")}><ArrowLeft size={19} /> Ścieżka nauki</button><div className="lesson-number">{String(lesson.id).padStart(2, "0")}</div><span className="lesson-kicker">Moduł {lesson.moduleId} · {lesson.checkpoint ? "checkpoint · " : ""}{lesson.duration} min</span><h1>{lesson.title}</h1><p>{lesson.goal}.</p><div className="lesson-progress-line"><i style={{ width: `${lesson.id}%` }} /><span>{lesson.id} / 100</span></div></div></section><div className="lesson-content page-width">
    <section className="lesson-block lesson-block--intro"><span className="eyebrow">01 · Sytuacja</span><h2>{practice.introTitle}</h2><p className="large-copy">{practice.introQuestion}</p><div className="speak-cue">{practice.isMessage ? <NotePencil size={24} /> : <Microphone size={24} />}<div><strong>{practice.cueTitle}</strong><span>{practice.cueNote}</span></div></div><Reveal label="Pokaż bezpieczny początek"><p className="model-answer">{phrases[0]}</p><button className="inline-audio" onClick={() => speak(phrases[0])}><Headphones size={17} /> Posłuchaj</button></Reveal></section>
    <section className="lesson-block"><span className="eyebrow">02 · Phrase pack</span><h2>{lesson.checkpoint ? "Sześć fraz, które prowadzą rozmowę." : phrases.length === 3 ? "Trzy zdania, które wykonują pracę." : `${phrases.length} zdań, które wykonują pracę.`}</h2><p>{lesson.checkpoint ? "Potraktuj je jak mapę rozmowy. Nie musisz zapamiętywać każdego słowa." : "Wybierz frazę, która najlepiej pasuje do momentu rozmowy lub wiadomości."}</p><div className="phrase-grid">{phrases.slice(0, 6).map((phrase, index) => <PhraseCard key={phrase} phrase={phrase} index={index} isSaved={saved(phrase)} onToggle={togglePhrase} onSpeak={speak} />)}</div></section>
    <section className="lesson-block lesson-block--paper"><span className="eyebrow">03 · Rozpoznaj moment</span><h2>Co powiesz w tej chwili?</h2><blockquote><span>Intencja</span>Chcesz {quizIntention}. Wybierz zdanie, które bezpośrednio pasuje do tej intencji.</blockquote><div className="choice-list">{choiceOptions.map((option) => { const isCorrect = option === quizAnswer; return <button key={option} aria-pressed={selectedChoice === option} className={selectedChoice === option ? (isCorrect ? "correct" : "incorrect") : ""} onClick={() => setSelectedChoice(option)}><span>{option}</span>{selectedChoice === option && (isCorrect ? <CheckCircle size={20} weight="fill" /> : <X size={19} />)}</button>; })}</div>{selectedChoice && <p className="choice-feedback" role="status">{selectedChoice === quizAnswer ? "Tak. To zdanie bezpośrednio realizuje tę intencję." : `To naturalna fraza, ale dotyczy innego momentu. Tutaj szukasz zdania, które pomoże ${quizIntention}.`}</p>}</section>
    <section className="lesson-block split-practice"><div><span className="eyebrow">{practice.transformEyebrow}</span><h2>{practice.transformTitle}</h2><p className="large-copy">{practice.transformInstruction}</p><div className="pause-card"><span>{practice.pauseSeconds}</span><p>{practice.pauseLabel.replace(`${practice.pauseSeconds} `, "")}</p></div></div><Reveal label="Porównaj z odpowiedziami" tone="blue"><div className="stacked-answers">{phrases.slice(0, 3).map((phrase) => <p key={phrase}>{phrase}</p>)}</div></Reveal></section>
    <section className="lesson-block lesson-block--dialogue"><span className="eyebrow">05 · Twoja kolej</span><h2>{practice.dialogueTitle}</h2><p className="dialogue-instruction">{practice.isMessage ? "Przeczytaj kontekst, napisz własną odpowiedź i wstaw ją do wymiany. Dopiero potem zobaczysz przykład z lekcji." : "Przeczytaj kontekst, powiedz odpowiedź na głos, a potem wpisz ją w brakujące miejsce. Dopiero wtedy zobaczysz przykład z lekcji."}</p><div className="dialogue">{dialogueLead.length ? dialogueLead.map(([speaker, text], index) => <div className={`dialogue-line ${speaker === "You" ? "recruiter" : "candidate"}`} key={`lead-${speaker}-${index}`}><span>{speaker}</span><p>{text}</p></div>) : <div className="dialogue-context"><span>Sytuacja</span><p>Twoim celem jest {lesson.goal}.</p></div>}{dialogueAnswer ? <><div className="dialogue-line recruiter dialogue-line--inserted"><span>You · Twoja wersja</span><p>{dialogueAnswer}</p></div>{dialogueFollowUp.map(([speaker, text], index) => { const isRecruiter = speaker === "You"; return <div className={`dialogue-line ${isRecruiter ? "recruiter" : "candidate"}`} key={`follow-up-${speaker}-${index}`}><span>{speaker}</span><p>{text}</p>{isRecruiter && <button onClick={() => speak(text)} aria-label={`Posłuchaj: ${text}`}><Play size={16} weight="fill" /></button>}</div>; })}<div className="dialogue-comparison"><div><span>Przykład z lekcji</span><p>{dialogueModelAnswer}</p></div><button className="inline-audio" onClick={() => speak(dialogueModelAnswer)}><Headphones size={17} /> Posłuchaj przykładu</button></div><div className="dialogue-result-actions"><button className="dialogue-edit" onClick={editDialogueAnswer}><NotePencil size={17} /> Zmień swoją odpowiedź</button><button className="dialogue-ai-button" onClick={() => openAnswerReview(dialogueAnswer)}><ChatCircleDots size={18} /> Niech ChatGPT oceni twoją odpowiedź</button></div><p className="dialogue-transfer" role="status"><Check size={17} weight="bold" /> Twoja wersja czeka niżej w sekcji „Twoja wersja”.</p></> : <form className="dialogue-compose" onSubmit={insertDialogueAnswer}><label htmlFor={`dialogue-answer-${lesson.id}`}><span>You</span><strong>{practice.isMessage ? "Twoja wiadomość" : "Twoja odpowiedź"}</strong></label><textarea id={`dialogue-answer-${lesson.id}`} value={dialogueDraft} onChange={(event) => setDialogueDraft(event.target.value)} placeholder={practice.isMessage ? "Napisz krótką odpowiedź po angielsku…" : "Wpisz to, co powiedziałaś po angielsku…"} /><div className="dialogue-compose-actions"><small>Możesz porównać ze wzorem albo poprosić ChatGPT o pełną ocenę.</small><div className="dialogue-compose-buttons"><button type="submit" disabled={!dialogueDraft.trim()}>Wstaw do rozmowy <ArrowRight size={17} /></button><button type="button" className="dialogue-ai-button" disabled={!dialogueDraft.trim()} onClick={() => openAnswerReview(dialogueDraft)}><ChatCircleDots size={18} /> Niech ChatGPT oceni twoją odpowiedź</button></div></div></form>}</div>{dialogueAnswer && <p className="coach-note"><Sparkle size={18} weight="fill" /><span><strong>Coach’s note:</strong> Porównaj intencję i naturalność, nie każde słowo. Jeśli twoja wersja była jasna, krótka i uprzejma, zadziałała.</span></p>}</section>
...
function CheatSheets({ navigate }) {
  const [selectedId, setSelectedId] = useState(cheatSheets[0].id); const [copiedPhrase, setCopiedPhrase] = useState(""); const selected = cheatSheets.find((sheet) => sheet.id === selectedId); const selectedLessons = selected.lessonIds.map(getLesson);
...
function MyPhrases({ progress, updateProgress, navigate }) {
  const ratings = progress.phraseRatings; const items = [...new Set([...progress.myPhrases, ...Object.keys(ratings).filter((phrase) => ratings[phrase] === "again" || ratings[phrase] === "hard")])]; const [custom, setCustom] = useState(""); const [copiedPhrase, setCopiedPhrase] = useState("");
