...
function PromptModal({ lesson, progress, onClose, initialMode = "roleplay", initialInput = "", reviewDialogue = null, reviewContext = {} }) {
  ...
  const hard = withoutPersonalContacts(Object.entries(progress.phraseSchedule || {}).filter(([, review]) => review.rating === "again" || review.rating === "hard").map(([phrase]) => phrase)).slice(0, 12);
  const favorites = withoutPersonalContacts(progress.myPhrases).slice(0, 12);
  const includesContactDetails = containsPersonalContact(input);
  const prompt = includesContactDetails ? "" : isAnswerReview ? answerReviewPrompt(lesson, input, reviewDialogue, hard, favorites, reviewContext) : lessonPrompt(lesson, mode, input, hard, favorites);
  useEffect(() => {
...
function LessonPage({ id, progress, updateProgress, navigate }) {
  ...
  const openAnswerReview = (answer, reviewContext = {}) => {
    const value = answer.trim();
    if (!value) return;
    const reviewDialogue = reviewContext.kind === "lesson-transform"
      ? [["You", reviewContext.reference || quizAnswer]]
      : miniDialogue;
    setPromptRequest({ initialMode: "check", initialInput: value, reviewDialogue, reviewContext });
  };
...
export function App() {
  ...
  else if (route === "practice") page = <DailyPractice progress={progress} updateProgress={updateProgress} navigate={navigate} onReviewAnswer={(lesson, answer, reviewContext = {}) => setDailyPrompt({ lesson, initialMode: "check", initialInput: answer, reviewDialogue: reviewContext.kind === "translation" ? [["You", reviewContext.reference || lesson.phrases[0]]] : getLessonDialogue(lesson), reviewContext })} />;
  ...
  return <div className="app-shell"><a className="skip-link" href="#main" onClick={(event) => { event.preventDefault(); const target = document.getElementById("main"); if (!target) return; target.setAttribute("tabindex", "-1"); target.focus({ preventScroll: true }); target.scrollIntoView({ block: "start" }); }}>Przejdź do treści</a><Header route={route} navigate={navigate} />{storageWarning && <div className="storage-warning" role="alert"><ShieldCheck size={18} /> {storageWarning}</div>}{page}<Footer />{dailyPrompt && <PromptModal lesson={dailyPrompt.lesson} progress={progress} onClose={() => setDailyPrompt(null)} initialMode={dailyPrompt.initialMode} initialInput={dailyPrompt.initialInput} reviewDialogue={dailyPrompt.reviewDialogue} reviewContext={dailyPrompt.reviewContext} />}</div>;
}
